...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        require(bool(settings.SECRET_KEY) and settings.SECRET_KEY != 'mozguest-local-dev-key-only', 'DJANGO_SECRET_KEY deve ser forte e diferente da chave local.')
        ...
        if storage_backend == 'local':
            ...
            require(bool(getattr(settings, 'AWS_STORAGE_BUCKET_NAME', '')), 'AWS_STORAGE_BUCKET_NAME obrigatório quando MOZGUEST_STORAGE_BACKEND=s3.')
            require(bool(getattr(settings, 'AWS_ACCESS_KEY_ID', '')), 'AWS_ACCESS_KEY_ID obrigatório quando MOZGUEST_STORAGE_BACKEND=s3.')
            require(bool(getattr(settings, 'AWS_SECRET_ACCESS_KEY', '')), 'AWS_SECRET_ACCESS_KEY obrigatório quando MOZGUEST_STORAGE_BACKEND=s3.')
        ...
        payment_mode = getattr(settings, 'MOZGUEST_PAYMENT_GATEWAY_MODE', 'sandbox')
        if payment_mode == 'live':
            warn(bool(getattr(settings, 'MOZGUEST_GATEWAY_CALLBACK_TOKEN', '')), 'MOZGUEST_GATEWAY_CALLBACK_TOKEN deve estar definido em live.')
            ...
            warn(False, 'MOZGUEST_PAYMENT_GATEWAY_MODE está em sandbox. Não movimenta dinheiro real.')

        self.stdout.write(self.style.MIGRATE_HEADING('MOZGUEST PRODUCTION CHECK'))
        self.stdout.write(f'Versão: {getattr(settings, "MOZGUEST_VERSION", "unknown")}')
        self.stdout.write(f'Ambiente: {getattr(settings, "MOZGUEST_ENVIRONMENT", "unknown")}')
